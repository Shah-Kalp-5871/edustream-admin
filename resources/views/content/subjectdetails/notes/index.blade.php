...
<div class="animate-fade-up">
    ...
    <div style="background: var(--surface); border: 1px solid var(--border); border-radius: var(--r-lg); overflow: hidden; box-shadow: var(--shadow-sm);">
        ...
        <div id="foldersList">
            ...
            <div class="file-row folder-row notes-grid" data-id="{{ $folder->id }}" onclick="window.location.href='{{ url('/content/notes/' . $id . '?folder_id=' . $folder->id) }}'">
    ...
    if (folderEl) {
        Sortable.create(folderEl, {
            ...
            onEnd: function() {
                ...
                fetch('{{ url("/content/notes/folders/reorder") }}', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                        'X-CSRF-TOKEN': '{{ csrf_token() }}'
