    ...
    <div id="fileList" style="background: var(--surface); border: 1px solid var(--border); border-radius: var(--r); overflow: hidden;">
        ...
        @if($folders->isEmpty() && $files->isEmpty())
            <div style="padding: 40px; text-align: center; color: var(--text-muted);">
                ...
                <p>No papers or folders found in this directory.</p>
            </div>
        @endif

        <!-- Folders -->
        @foreach($folders as $folder)
        <div class="file-row folder-row" onclick="window.location.href='{{ url('/content/qa-papers/' . $id . '?folder_id=' . $folder->id) }}'">
        ...
        <div class="file-row">
            ...
            <div style="color: var(--text-muted); text-transform: uppercase;">{{ $paper->file_type }}</div>
